            ...
            {open && (
                <div className={`absolute right-0 top-full mt-1.5 w-64 rounded-xl border shadow-xl overflow-hidden z-50 ${
                    ...
                    <div className="py-1.5">
                        ...
                        <a href={`https://github.com/pyxle-framework/pyxle-dev/blob/main/public/docs-data/${page.path}.json`} target="_blank" rel="noreferrer" onClick={() => setOpen(false)} className={`w-full flex items-start gap-3 px-4 py-2.5 text-left transition no-underline ${theme === 'dark' ? 'hover:bg-white/5' : 'hover:bg-zinc-50'}`}>
                            <svg className={`h-4 w-4 mt-0.5 shrink-0 ${theme === 'dark' ? 'text-zinc-500' : 'text-zinc-400'}`} fill="currentColor" viewBox="0 0 24 24"><path d="M12 0C5.37 0 0 5.37 0 12c0 5.31 3.435 9.795 8.205 11.385.6.105.825-.255.825-.57 0-.285-.015-1.23-.015-2.235-3.015.555-3.795-.735-4.035-1.41-.135-.345-.72-1.41-1.23-1.695-.42-.225-1.02-.78-.015-.795.945-.015 1.62.87 1.845 1.23 1.08 1.815 2.805 1.305 3.495.99.105-.78.42-1.305.765-1.605-2.67-.3-5.46-1.335-5.46-5.925 0-1.305.465-2.385 1.23-3.225-.12-.3-.54-1.53.12-3.18 0 0 1.005-.315 3.3 1.23.96-.27 1.98-.405 3-.405s2.04.135 3 .405c2.295-1.56 3.3-1.23 3.3-1.23.66 1.65.24 2.88.12 3.18.765.84 1.23 1.905 1.23 3.225 0 4.605-2.805 5.625-5.475 5.925.435.375.81 1.095.81 2.22 0 1.605-.015 2.895-.015 3.3 0 .315.225.69.825.57A12.02 12.02 0 0024 12c0-6.63-5.37-12-12-12z"/></svg>
                            <div className="min-w-0 flex-1">
                                <div className="flex items-center gap-1.5">
                                    <p className={`text-sm font-medium ${theme === 'dark' ? 'text-zinc-200' : 'text-zinc-700'}`}>View source on GitHub</p>
                                    <svg className={`h-3 w-3 ${theme === 'dark' ? 'text-zinc-600' : 'text-zinc-300'}`} fill="none" viewBox="0 0 24 24" strokeWidth="2" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" d="M13.5 6H5.25A2.25 2.25 0 003 8.25v10.5A2.25 2.25 0 005.25 21h10.5A2.25 2.25 0 0018 18.75V10.5m-10.5 6L21 3m0 0h-5.25M21 3v5.25" /></svg>
                                </div>
                                <p className={`text-xs ${theme === 'dark' ? 'text-zinc-500' : 'text-zinc-400'}`}>See this page's data on GitHub</p>
                            </div>
                        </a>
                    </div>
                </div>
    ...
    return (
        <div className="min-h-screen">
            ...
            <header className={`sticky top-0 z-50 border-b backdrop-blur-xl ${
                ...
                <div className="mx-auto flex max-w-[90rem] items-center justify-between px-4 sm:px-6 py-3">
                    ...
                    <div className="flex items-center gap-2 sm:gap-3">
                        ...
                        <a
                            href="https://github.com/pyxle-framework/pyxle-dev/blob/main/pages/docs/%5B%5B...slug%5D%5D.pyx"
                            target="_blank"
                            rel="noreferrer"
            ...
            <style dangerouslySetInnerHTML={{ __html: `
                ...
                .docs-content table { display: block; overflow-x: auto; -webkit-overflow-scrolling: touch; width: calc(100% - 1.4rem); margin-bottom: 1.25rem; margin-left: 1.4rem; border-collapse: collapse; font-size: 0.875rem; }
